...
export default function UpdateServiceModal({ isOpen, onClose, service, onUpdate, locale, t }) {
    ...
    // Populate form data when a service is selected
    useEffect(() => {
        if (isOpen && service) {
            setFormData({
                name: {
            ...
            <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
                <div className="bg-white p-6 w-full max-w-lg relative">
                    <button onClick={() => {
                        ...
                        <Image src={closeIcon} width={24} height={24} alt={t.close} />
                    </button>
                    <h2 className="text-2xl mb-4 font-bold">{t.updateService}</h2>
                    ...
                    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                        <label className="text-gray-700">
                            ...
                            <input
                                ...
                                required
                                className="w-full border p-2 rounded-lg text-black mt-1"
                            />
                        </label>
                        <label className="text-gray-700">
                            {t.nameUz}
